...
<img src="/logos/github.svg" class="inset-right" /> GitHub makes it really easy to host a static content website like this one. Just create a repo named `your-user-name.github.io` and its contents will be hosted at `http://your-user-name.github.io`. [Project-specific sites][project-pages] are also available.
...
<img src="/logos/nodejs.svg" class="inset-right" /> As it's such a simple site, it could easily be hosted anywhere. It's especially easy with Node. Make sure you have [Git][git] and [Node.js][node] installed. Then run the following:

  * `git clone https://github.com/rkoeninger/rkoeninger.github.io.git`
  * `cd rkoeninger.github.io`
...
<img src="/logos/git.svg" class="inset-right" /> I have this principle for source control where one should not check-in files generated by or from other files in the repo without a really good reason.
...
<img src="/logos/html.svg" class="inset-right" /> The site has a single HTML file which frames the current article, links stylesheets and javascript files and show the basic navigation bar. It uses the modern convention of Semantic Markup, using `<main>`, `<article>` and `<nav>` tags instead of `<div>`s for everything.
...
MathJax makes for beautifully typeset 
...
<img src="/logos/svg.svg" class="inset-right" /> Some of the SVG images I have vectorized from raster images and some I have drawn myself. My originals are distinguished by their crappiness. I am not an illustrator or graphic designer before, but it's nice to play with it from time to time.
...
<img src="/logos/fearOfABlueScreen.svg" class="inset-right" /> With the home page's logo, I hit a couple of road blocks.
