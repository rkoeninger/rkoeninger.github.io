...
![](/images/robby.png) Working on this site has served a dual purpose for me. It gives me a place to write up ideas I've had so I can easily present them to people and it gives me a project to work on where I can learn about web development.
...
![](/logos/github.svg) GitHub makes it really easy to host a static content website like this one. Just create a repo named `your-user-name.github.io` and its contents will be hosted at `http://your-user-name.github.io`. [Project-specific sites][project-pages] are also available.
...
![](/logos/nodejs.svg) As it's such a simple site, it could easily be hosted anywhere. It's especially easy with Node.
<span class="no-mobile">Make sure you have [Git][git] and [Node.js][node] installed. Then run the following:</span>
<span class="only-mobile">You can just get the `http-server` package through npm.</span>

<div class="no-mobile">
  <ol>
    <li>`git clone https://github.com/rkoeninger/rkoeninger.github.io.git`</li>
    <li>`cd rkoeninger.github.io`</li>
    <li>`npm install -g http-server`</li>
    <li>`http-server -p 8080`</li>
    <li>open <a href="//localhost:8080">localhost:8080</a> in your browser</li>
  </ol>
</div>
...
![](/logos/markdown.svg) I originally tried writing articles in straight-html, but after the first 1000 `</>`s, my pinky got tired. Having everything explicitly delimited is great in programming languages, but when I'm working in natural language, which already has it's own syntax, I'd like there to be a minimum of noise.
...
![](/logos/polymer_icon.svg) As my article content became more involved, I found myself writing out piles of HTML, getting no help from the Markdown renderer.

#### Source Control

![](/logos/git.svg) I have this principle for source control where one should not check-in files generated by or from other files in the repo without a really good reason.

From what I've read, the typical way to write a GitHub Pages site is to write your content in Markdown and use command-line tools to generate HTML files and then check in both changed files. This violates my first (and only, really) rule of source control.
...
![](/logos/html.svg) The site has a single HTML file which frames the current article, links stylesheets and javascript files and show the basic navigation bar. It uses the modern convention of Semantic Markup, using `<main>`, `<article>` and `<nav>` tags instead of `<div>`s for everything.

#### AJAX / PJAX

![](/logos/javascript.svg) So we can convert the Markdown to HTML, but you can't just link to a Markdown file and expect anything to happen. You have to load an HTML file.
...
![](/logos/css.svg) I've made some attempt at making the site responsive. The hardest part has been just getting the font sizes right.
...
![](/logos/jquery_icon.svg) Pretty standard use of JQuery. Selecting and modifying elements, attaching event listeners, creating elements, etc.
...
<div class="no-mobile">
  <pre><code class="lang-html">&lt;a href="..." class="fa fa-home"     id="home-link"&gt;&lt;/a&gt;
&lt;a href="..." class="fa fa-code"     id="source-link"&gt;&lt;/a&gt;
&lt;a href="..." class="fa fa-calendar" id="history-link"&gt;&lt;/a&gt;</code></pre>
</div>
...
![](/logos/mathJax.svg) MathJax makes for beautifully typeset mathematical formulas and expressions. My article on [the golden ratio][golden] makes extensive use of this library.
...
![](/logos/svg.svg) Some of the SVG images I have vectorized from raster images and some I have drawn myself. My originals are distinguished by their crappiness. I am not an illustrator or graphic designer before, but it's nice to play with it from time to time.
...
![](/logos/fearOfABlueScreen.svg) With the home page's logo, I hit a couple of road blocks.
...
![](/logos/requirejs_icon.svg) Javascript modules are loaded using AMD via Require.js. Different configurations are used for testing and the live site. Shims were needed for MathJax and Highlight.js.
...
![](/logos/jasmine.svg) The spec page can be viewed any time at [/spec.html][specpage]
